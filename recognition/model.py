...
class Model:
	...
	def __init__(self, charList, decoderType=DecoderType.BestPath, mustRestore=False, dump=False):
		...
		with self.graph.as_default():
			...
			modelDir = os.path.join(FILE_PATH, '../model/')
	...
	def setupCTC(self):
		...
		if self.decoderType == DecoderType.BestPath:
			...
			word_beam_search_module = tf.load_op_library(os.path.join(FILE_PATH, '../TFWordBeamSearch.so'))

			# prepare information about language (dictionary, characters in dataset, characters forming words)
			chars = str().join(self.charList)
			wordChars = open(os.path.join(FILE_PATH, '../model/wordCharList.txt')).read().splitlines()[0]
			corpus = open(os.path.join(FILE_PATH, '../data/words_case.txt')).read()

			# decode using the "Words" mode of word beam search
			self.decoder = word_beam_search_module.word_beam_search(tf.nn.softmax(self.ctcIn3dTBC, dim=2), 50, 'NGrams', 0.0, corpus.encode('utf8'), chars.encode('utf8'), wordChars.encode('utf8'))
	...
	def dumpNNOutput(self, rnnOutput):
		...
		dumpDir = os.path.join(FILE_PATH, '../dump/')
	...
	def save(self):
		"save model to file"
		self.snapID += 1
		self.saver.save(self.sess, os.path.join(FILE_PATH, '../model/snapshot'), global_step=self.snapID)
